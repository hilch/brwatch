...
Module   : pvi_interface.c
 Created  : Thu Feb  2 09:53:29 2006
 Modified : Thu Feb  2 09:53:29 2006

 Author   : B&R-TB-West CHilchenbach
 Project  : Pvi- Interface
...
static PVIOBJECT *RemovePviObject(PVIOBJECT * object )
{
	...
	while( prevobject->next != NULL ){
		...
		if( find == object ){
			result = PviUnlink(object->linkid);	// Pvi- Objekt unlinken
			object->linkid = 0;
				...
				if( object->type == POBJ_PVAR ){					
					switch( object->ex.pv.type ){
						case BR_STRING:
							...
							if( object->ex.pv.pvalue != NULL )
								memcpy( object->ex.pv.pvalue, pData, DataLen );
							break;
...
int StartPvi( void ){
	...
	timerid_cyclic_requests = SetTimer( NULL, 0, 500, PviCyclicRequests );
			...
			if( FindToken( &s, "{" ) ){   // Beginn einer Elementdefinition
				...
				*d = 0;
				...
				if( CountToken( tempstring, '.' ) > 1 ){
					while( !FindToken( &s, "}" ) )++s;  // alles überlesen
					element_definition = FALSE;
				}
				continue;
			}
	...
	while (parent != NULL) {
		if ( IsPviObjectParentOf(parent, object) && strrchr(parent->name, '.') == NULL ) {	// Eltern suchen 
			...
			if (section != NULL) {
				...
				BOOL found = FALSE;
				tempstring[0] = 0;
				result = GetPrivateProfileString( section, section, "0", tempstring, sizeof(tempstring), filename );
