...
int GetPviObjectImage( PVIOBJECT *object ){
	...
	switch( object->type ){
		...
		switch( object->ex.pv.type ){
			case BR_STRUCT: imageindex = GetImageIndex(IDR_ICO_STRUCT); break;
			case BR_UDINT: 	imageindex = GetImageIndex(IDR_ICO_VARIABLE_U32); break; 
			case BR_DINT: 	imageindex = GetImageIndex(IDR_ICO_VARIABLE_I32); break; 
			case BR_UINT: 	imageindex = GetImageIndex(IDR_ICO_VARIABLE_U16); break;
			case BR_INT: 	imageindex = GetImageIndex(IDR_ICO_VARIABLE_I16); break;
			case BR_USINT: 	imageindex = GetImageIndex(IDR_ICO_VARIABLE_U8); break;
			case BR_SINT: 	imageindex = GetImageIndex(IDR_ICO_VARIABLE_I8); break; 
			case BR_BOOL:	imageindex = GetImageIndex(IDR_ICO_VARIABLE_BOOL); break; 
			case BR_REAL:	imageindex = GetImageIndex(IDR_ICO_VARIABLE_REAL); break;
			case BR_STRING: imageindex = GetImageIndex(IDR_ICO_VARIABLE_STRING); break;
			case BR_TIME: 	imageindex = GetImageIndex(IDR_ICO_VARIABLE_TIME); break;
			case BR_DATI: 	imageindex = GetImageIndex(IDR_ICO_VARIABLE_DATE_AND_TIME); break;
			default: imageindex = GetImageIndex(IDR_ICO_VARIABLE); break;			
		}
		break;
...
{
	PVIOBJECT *object;
	RECT rect_window, rect_item;
	HCURSOR hcursor;
	unsigned long broadcast;

	if( GetWindowRect( mytreeviewparam.hwndTV, &rect_window ) ){
		if( TreeView_GetItemRect( mytreeviewparam.hwndTV, hti, &rect_item, TRUE ) ){
							...
							case BR_STRUCT:
								...
								if( object->ex.pv.pdatatype != NULL ){
									...
									if( !strcmp( object->ex.pv.pdatatype, "wh_acptrace") ){
										...
										DialogBoxParam( ghInstance, MAKEINTRESOURCE(DLG_AXCONFIG), GetMainWindow(), ConfigAxTraceDlg, (LPARAM) object );
									}
								}
								break;
						}
		...
		if( object->type == POBJ_PVAR){
			...
			if( WatchPviObject( object, TRUE ) != NULL ) {
				...
				if( object->ex.pv.type == BR_STRING ){
					sprintf( tempstring, "%1s%s(%u)", object->ex.pv.scope[0] == 'd' ? "*" : "", object->ex.pv.pdatatype, object->ex.pv.length-1 ); 
				}
				else {
				...
				switch( object->ex.pv.scope[0] ){
					case 'g': text = "<GLOBAL>"; break;
					case 'l': text = "<LOCAL >"; break;
					case 'd': text = "<DYNAM.>"; break;
				}
				...
				if( lvitem.lParam != 0 ) {
					...
					if( object->type == POBJ_PVAR ){
						DialogBoxParam( ghInstance, MAKEINTRESOURCE(DLG_WRITE_PVAR), hMainWindow, 
								WritePvarDlgProc, (LPARAM) object );
					}
				}
			}
			break;
